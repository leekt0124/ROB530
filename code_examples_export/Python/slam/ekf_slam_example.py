...
if __name__ == "__main__":
    ...
    # for new landmarks seen_landmarks has to be expanded
    ...
    for i in range(pose_number):
        ...
        if len(z['correspondences'][i]):
            for j in range(len(z['correspondences'][i])):
                ...
                # if not seen, append landmark
                if not seen:
                    seen_landmarks.append(l_id)
                    key = len(seen_landmarks) - 1
                    ekf['predict_state'][2+2*key:4+2*key] = ekf['state'][0:2] + z['landmark_coord'][i][j, :].reshape(
                        2, 1)
                    ekf['predict_cov'][2+2*key:4+2*key, 2+2*key:2*key+4] = ekf['cov'][0:2, 0:2] + sigma_z  # Sigma_ll
                    ekf['predict_cov'][2+2*key:4+2*key, 0:2] = ekf['cov'][0:2, 0:2]  # Sigma_lr
                    ekf['predict_cov'][2+2*key:4+2*key, 2:2+2*key] = (ekf['cov'][0:2, 2:2+2*key])  # Sigma_lL
                    ekf['predict_cov'][2:2+2*key, 2+2*key:4+2*key] = (ekf['cov'][2+2*key:4+2*key, 2:2+2*key]).T  # Sigma_Ll
                    ekf['predict_cov'][0:2, 2+2*key:4+2*key] = (ekf['cov'][2+2*key:4+2*key, 0:2]).T  # Sigma_rl
                    ekf['state'] = np.copy(ekf['predict_state'])
                    ekf['cov'] = np.copy(ekf['predict_cov'])
                # if seen, add a loop-closure
                else:
                    key = seen_landmarks.index(l_id)
                    # assemble the Jacobian for the jth measurement
                    Hj = np.zeros([2, ekf['state_dim']])
                    Hj[:, 0:2] = H
                    Hj[:, 2 + 2 * key:4 + 2 * key] = J
                    ...
                    # apply correction
                    ekf['predict_state'] = ekf['predict_state'] + np.dot(K, v)
                    temp = np.eye(np.dot(K, Hj).shape[0]) - np.dot(K, Hj)
                    ekf['predict_cov'] = np.dot(np.dot(temp, ekf['predict_cov']), temp.T) + np.dot(np.dot(K, sigma_z), K.T)
                    ekf['state'] = np.copy(ekf['predict_state'])
                    ekf['cov'] = np.copy(ekf['predict_cov'])
